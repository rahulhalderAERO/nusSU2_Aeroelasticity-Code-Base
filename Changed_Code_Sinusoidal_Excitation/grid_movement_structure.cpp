      ...
      for (jMarker = 0; jMarker<config->GetnMarker_Moving(); jMarker++) {
        ...
        if (Marker_Tag == Moving_Tag) {
          ...
          for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
            ...
            change_coord[2] = -0.01*((Coeff[0] + Coeff[1]*Coord[0]+Coeff[2]*Coord[1]+Coeff[3]*pow(Coord[0],2)+Coeff[4]*Coord[0]*Coord[1]+Coeff[5]*pow(Coord[1],2)+Coeff[6]*pow(Coord[0],3)+Coeff[7]*pow(Coord[0],2)*Coord[1]+Coeff[8]*pow(Coord[1],2)*Coord[0]+Coeff[9]*pow(Coord[1],3)))*(sin(100*time_new) - sin(100*time_old))+Coord[2];
